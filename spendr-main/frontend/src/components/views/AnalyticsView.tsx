...
export const AnalyticsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-[#121216] border border-white/10 rounded-2xl p-6">
          ...
          <div className="mt-2 w-full bg-[#1A1A22] h-2 rounded-full overflow-hidden">
            <div className="bg-[#10B981] h-full rounded-full" style={{ width: `${budgetUsedPct}%` }} />
          </div>
          <p className="text-xs text-[#8888A0] mt-1 flex justify-between">
            <span>Used: {budgetUsedPct}%</span>
            <span className="text-[#34D399] font-mono">${remaining_budget.toFixed(2)} left</span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#121216] border border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-xs font-mono text-[#8888A0] bg-[#1A1A22] px-3 py-1 rounded-xl border border-white/10">
              ...
              <BarChart data={category_distribution} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
                ...
                <Bar dataKey="amount" radius={[6, 6, 0, 0]}>
                  {category_distribution.map((_, index) => (
                    <Cell key={`bar-${index}`} fill={index === 0 ? '#10B981' : index === 1 ? '#444458' : '#2A2A38'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div>
            ...
            <div className="space-y-3">
              {top_impact_subscriptions.map((sub, idx) => (
                <div key={sub.id} className="p-3 bg-[#0A0A0C] border border-white/5 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-[#10B981]/15 border border-[#10B981]/30 flex items-center justify-center font-bold text-xs text-[#34D399]">
                      #{idx + 1}
                    </div>
                    <div>
      ...
      <div className="bg-[#121216] border border-[#10B981]/30 rounded-2xl p-6 relative overflow-hidden shadow-brand-glow">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-2xl bg-[#10B981] flex items-center justify-center text-white shrink-0">
            <Sparkles className="w-5 h-5 fill-current" />
          </div>
          <div className="space-y-1 flex-1">
            <div className="flex items-center justify-between">
              <h4 className="text-base font-bold text-white">Vaultly Intelligence Insights</h4>
              <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-[#10B981]/20 border border-[#10B981]/40 text-[#34D399]">
                Optimization Active
              </span>
            </div>
