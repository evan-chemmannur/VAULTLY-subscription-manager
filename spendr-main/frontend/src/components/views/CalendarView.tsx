    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#121216] border border-white/10 rounded-2xl p-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-[#10B981]/15 border border-[#10B981]/30 flex items-center justify-center text-[#34D399]">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={prevMonth}
            className="p-2 bg-[#1A1A22] hover:bg-white/10 text-white rounded-xl border border-white/10 transition-colors"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button
            onClick={resetToday}
            className="px-3 py-1.5 bg-[#1A1A22] hover:bg-white/10 text-xs font-semibold text-white rounded-xl border border-white/10 transition-colors"
          >
            Today
          </button>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 bg-[#121216] border border-white/10 rounded-2xl p-6 shadow-xl">
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {Array.from({ length: firstDayOfWeek }).map((_, idx) => (
              <div key={`empty-${idx}`} className="h-24 bg-[#0A0A0C]/50 rounded-2xl border border-white/5 opacity-30" />
            ))}
              ...
              return (
                <div
                  ...
                  className={`h-24 p-2 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between ${
                    isSelected
                      ? 'bg-[#1A1A22] border-[#10B981] ring-1 ring-[#10B981] shadow-brand-glow'
                      : hasSubs
                      ? 'bg-[#0A0A0C] border-[#10B981]/40 hover:border-[#10B981]'
                      : 'bg-[#0A0A0C] border-white/5 hover:border-white/20'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-mono font-bold ${hasSubs ? 'text-[#34D399]' : 'text-[#8888A0]'}`}>
                      {dayNum}
                    </span>
                    {hasSubs && (
                      <span className="w-1.5 h-1.5 rounded-full bg-[#10B981] shadow-brand-glow" />
                    )}
                  </div>
                  ...
                  <div className="space-y-1 overflow-hidden">
                    {daySubs.slice(0, 2).map((s) => (
                      <div key={s.id} className="px-1.5 py-0.5 bg-[#10B981]/20 border border-[#10B981]/40 rounded text-[10px] font-medium text-white truncate">
        ...
        <div className="bg-[#121216] border border-white/10 rounded-2xl p-6 flex flex-col justify-between">
              ...
              selectedSubs.length > 0 ? (
                <div className="space-y-3">
                  {selectedSubs.map((sub) => (
                    <div key={sub.id} className="p-3 bg-[#0A0A0C] border border-[#10B981]/30 rounded-2xl space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-white">{sub.name}</span>
                        <span className="text-xs font-mono font-bold text-[#34D399]">${Number(sub.amount).toFixed(2)}</span>
                      </div>
                ...
                <div className="p-6 bg-[#0A0A0C] border border-white/5 rounded-2xl text-center text-xs text-[#8888A0]">
          ...
          <button
            ...
            className="w-full mt-6 py-2.5 bg-[#10B981] hover:bg-[#34D399] text-white text-xs font-medium rounded-2xl shadow-brand-glow transition-all flex items-center justify-center gap-1.5"
          >
