      ...
      case 'Active':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-white/10 text-white border border-white/15">
            <CheckCircle2 className="w-3 h-3 text-[#10B981]" /> Active
          </span>
        );
      case 'Upcoming':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-[#10B981]/15 text-[#34D399] border border-[#10B981]/30">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#121216] border border-white/10 rounded-2xl p-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-[#10B981] hover:bg-[#34D399] text-white text-sm font-medium rounded-2xl shadow-brand-glow transition-all hover:scale-105 active:scale-95"
          >
        ...
        {['All', 'Software', 'Streaming', 'Cloud', 'Infrastructure', 'Utilities', 'Fitness'].map((cat) => (
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all ${
              categoryFilter === cat
                ? 'bg-[#1A1A22] text-white border border-[#10B981]/40 shadow-brand-glow'
                ...
                {sorted.map((sub) => (
                  <tr key={sub.id} className="hover:bg-[#1A1A22] transition-colors group">
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-[#1A1A22] border border-white/10 flex items-center justify-center text-white font-bold text-sm uppercase shadow-sm">
                          {sub.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-semibold text-white group-hover:text-[#10B981] transition-colors">{sub.name}</p>
                    ...
                    <td className="py-4 px-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-2 bg-white/5 hover:bg-white/15 text-[#E0E0E0] hover:text-white rounded-xl transition-colors"
                        ...
                        <button
                          ...
                          className="p-2 bg-[#10B981]/15 hover:bg-[#10B981]/30 text-[#34D399] rounded-xl transition-colors"
